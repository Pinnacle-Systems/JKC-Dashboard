...
const StateWiseDetails = ({
  ...
  console.log(HeadData, "HeadDataintable");
  ...
  const filteredData = Array.isArray(HeadData)
    ? HeadData.filter((row) =>
        Object.keys(search || {}).every((key) => {
          ...
          // Add this block inside the Object.keys(search).every() callback,
          // before the generic return at the bottom:
  ...
  const downloadExcel = async () => {
    ...
    saveAs(new Blob([buffer]), "State Wise Headcount Report.xlsx");
  };
  return (
