...
/* ── Birthday check: same DD-MM as today ── */
const today = moment();
...
const AttendenceDistributionTable = ({
  ...
  const handleExport = async () => {
    ...
    // Map COLUMNS def → ExcelJS columns (skip sno, add it manually)
    ...
    /* ── Row 1: Title ── */
    ws.insertRow(1, ["Attendance Distribution"]);
    ...
    /* ── Row 2: Insights (params) ── */
    addInsightsRowTurnOver({
      worksheet: ws,
      startRow: 2,
      totalColumns: colCount,
      selectedYear: selectedDate, // date used as label
    ...
    /* ── Data rows (start at row 4 because insertRow pushed header to 3) ── */
    ...
    /* ── Style data rows per column alignment ── */
    ws.eachRow((row, rn) => {
      if (rn <= 3) return;
      row.height = 22;
